...
function initTheme() {
  applyTheme(localStorage.getItem('scaffale-theme') || 'dark');
}
$('#theme-toggle').addEventListener('click', () => {
  const next = document.body.dataset.theme === 'dark' ? 'light' : 'dark';
  applyTheme(next);
  localStorage.setItem('scaffale-theme', next);
});
...
function renderGrid() {
  ...
  let list = state.books.slice().sort((a, b) => (b.lastOpenedAt || b.addedAt) - (a.lastOpenedAt || a.addedAt));
...
// swipe support in paginated mode
(() => {
  let startX = null;
